...
- [ ] ファイルパス・URL・ポート番号・ホスト名などの環境依存値がソースに直書きされていないか（env / 設定ファイル経由になっているか。個人フォルダパス・絶対パスの直書き含む、RULE_CODE §6）
- [ ] 過度にネストした条件分岐がないか (早期リターンで改善可能か)
- [ ] 未使用のコード・デッドコードが残存していないか
- [ ] コピー&ペーストによる重複コードがないか (DRY違反)
- [ ] 新設前に Anatomia の解析グラフで再利用候補を探し、候補の採否と理由を PR 説明に残しているか (RULE_CODE §6)
