...
- [ ] 実装前に Anatomia の `test-suggestions` を入力として Augur でテストを計画し、実装とテストを同じ変更単位に入れているか (RULE_TEST §2)
- [ ] 計画からテストを減らした場合、その理由を PR 説明に残しているか (RULE_TEST §2)
- [ ] 変更種別に応じた既存テストが `kinds` で選別・実行され、回帰が green か (RULE_TEST §4)
