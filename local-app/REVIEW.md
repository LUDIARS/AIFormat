...
デスクトップ / モバイル / CLI 等のローカル実行アプリ向けのレビューフォーマット。
...
| ドキュメント | 含まれるレビュー観点 |
|------------|-------------------|
| [設計レビュー](../common/REVIEW_DESIGN.md) | **ドメイン紐づけ(最初に見る)** / 設計強度 / 設計思想の一貫性 / モジュール分割度 |
...
| [ローカル品質保証レビュー](REVIEW_QUALITY_LOCAL.md) | パフォーマンス / クロスプラットフォーム / アクセシビリティ・国際化 |
...
| 14 | センシティブ情報取扱いポリシー | ローカル | A~D | | [ポリシーレビュー](REVIEW_PRIVACY_POLICY.md) |
| 15 | ローカル攻撃面 | ローカル | A~D | | [ローカル脆弱性レビュー](REVIEW_VULNERABILITY_LOCAL.md) |
| 16 | 権限・サンドボックス | ローカル | A~D | | [ローカル脆弱性レビュー](REVIEW_VULNERABILITY_LOCAL.md) |
| 17 | IPC・ローカルサーバ | ローカル | A~D | | [ローカル脆弱性レビュー](REVIEW_VULNERABILITY_LOCAL.md) |
| 18 | アップデート・配布の完全性 | ローカル | A~D | | [ローカル脆弱性レビュー](REVIEW_VULNERABILITY_LOCAL.md) |
| 19 | アンチタンパリング | ローカル | A~D | | [ローカル脆弱性レビュー](REVIEW_VULNERABILITY_LOCAL.md) |
| 20 | ローカルデータストア | ローカル | A~D | | [ローカル実装評価](REVIEW_IMPLEMENTATION_LOCAL.md) |
| 21 | バックアップ・移行 | ローカル | A~D | | [ローカル実装評価](REVIEW_IMPLEMENTATION_LOCAL.md) |
| 22 | 運用信頼性 | ローカル | A~D | | [ローカル実装評価](REVIEW_IMPLEMENTATION_LOCAL.md) |
| 23 | パフォーマンス | ローカル | A~D | | [ローカル品質保証レビュー](REVIEW_QUALITY_LOCAL.md) |
| 24 | クロスプラットフォーム互換 | ローカル | A~D | | [ローカル品質保証レビュー](REVIEW_QUALITY_LOCAL.md) |
| 25 | アクセシビリティ・国際化 | ローカル | A~D | | [ローカル品質保証レビュー](REVIEW_QUALITY_LOCAL.md) |

> センシティブ情報取扱いポリシー (行 14) は、ポリシーレビュー内 8 観点の指摘を
> まとめて最悪重大度から導出する**集約行**とする (8 観点の個別評価はドキュメント側で管理)。
> LLM 機能のセキュリティ (行 10) は LLM 機能が無い場合に「対象外」とする。
