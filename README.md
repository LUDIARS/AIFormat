...
- [`RULE.md`](./RULE.md) — 基盤設計ルール (認証 / DB マイグレーション / マイクロサービス / 個人データ / worktree / GitHub リリース運用)
...
- [`RULE_TEST.md`](./RULE_TEST.md) — **テスト** (リリース前のローカル検証必須 / アーキ別にコンテンツごとのテスト設計 / テスト充実度を品質基準に)
- [`RULE_SRE.md`](./RULE_SRE.md) — **SRE・運用** (種別別の運用 watchpoint / ヘルスチェック `/api/health` / Issue 報告)

## レビュー (見るとき)

- [`REVIEW.md`](./REVIEW.md) — レビュー全体の入口
- [`prompt/`](./prompt/) — 実行プロンプト集 (フルレビュー / PR 差分レビュー / 仕様欠落検知・spec 自動生成)
- [`script_design/`](./script_design/) — 機械解析スクリプトの設計 (ローカル検証ゲート / Anatomia 計装の振り分けと検知ロジック)
- [`scripts/LEAK_CHECKER.md`](./scripts/LEAK_CHECKER.md) — ローカル漏洩走査と GitHub public/private 境界監査
...
- [`FORMAT_UX.md`](./FORMAT_UX.md) — プロダクトの問題と勝ちを定義し、Anatomia / Praeforma が共通参照する UX 定義書
